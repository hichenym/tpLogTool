...
VERSION_MINOR = 1  # 次版本号
VERSION_PATCH = 1  # 修订号
BUILD_DATE = "20260117"  # 编译日期
```
...
3. 运行 `python build.py` 自动更新编译日期并打包
...
- `-i ./icon/logo.ico`: 指定程序图标
- `--name "设备查询工具"`: 指定生成的exe文件名
...
**解决**：
1. 确保 `icon/logo.ico` 文件存在
2. 确保是标准的 .ico 格式
3. 重新生成资源文件：`pyrcc5 icon_res.qrc -o icon_res.py`

### Q5: 编译日期没有更新

**原因**：未使用自动打包脚本

**解决**：使用 `python build.py` 自动更新编译日期
